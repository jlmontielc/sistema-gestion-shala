...
def crear_clase():
    instructores_query = Usuario.query.filter_by(rol="INSTRUCTOR").join(
        Usuario.instructor
    )
    if current_user.rol == "ADMIN_SHALA" and current_user.shala_id:
        instructores_query = instructores_query.filter(
    ...
    if request.method == "POST":
        ...
        instructor = (
            Usuario.query.filter_by(id=int(instructor_id), rol="INSTRUCTOR").first()
            if instructor_id
            else None
        )
