...
def crear_shala():
    ...
    if request.method == "POST":
        ...
        nueva_shala = Shala(
            ...
            logo_url=logo_url,
            admin_shala_id=(
                current_user.id if current_user.rol == "ADMIN_SHALA" else None
            ),
        )
...
def detalle_shala(id):
    ...
    instructores = (
        Usuario.query.join(Instructor, Instructor.id == Usuario.id)
        .filter(Usuario.rol == "INSTRUCTOR", Instructor.shala_id == id)
