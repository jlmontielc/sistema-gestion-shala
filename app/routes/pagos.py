...
def iniciar_pago(clase_id):
    ...
    session = stripe.checkout.Session.create(
        ...
        metadata={"usuario_id": current_user.id, "clase_id": clase.id},
    )
...
def crear_sesion_pago():
    ...
    session = stripe.checkout.Session.create(
        ...
        + "?session_id={CHECKOUT_SESSION_ID}",
        cancel_url=url_for("pagos.pago_cancelado", _external=True),
        metadata={"usuario_id": current_user.id, "paquete_id": paquete.id},
    )
...
def pago_exitoso():
    ...
    if metadata.get("paquete_id"):
        ...
        if paquete:
            current_user.saldo_clases = (current_user.saldo_clases or 0) + (
                paquete.sesiones_incluidas or 0
            )
            db.session.commit()
