...
def dashboard():
    ...
    if current_user.rol == "ADMIN_SHALA":
        ...
        reservas_activas = Reserva.query.filter(
            Reserva.estado == "RESERVADO", Reserva.clase_id.in_(clases_ids)
        ).count()
        pagos_completados = (
            db.session.query(db.func.sum(Pago.monto))
            .join(Paquete, Paquete.id == Pago.paquete_id)
            .filter(
                Pago.estado == "COMPLETADO", Paquete.shala_id == current_user.shala_id
            )
            .scalar()
            or 0
        )
        usuarios_yoguis = Usuario.query.filter_by(
            rol="YOGUI", shala_id=current_user.shala_id
        ).count()
        usuarios_instructores = (
            Usuario.query.join(Usuario.instructor)
            .filter_by(shala_id=current_user.shala_id)
            .count()
        )
        ...
        clases_data = (
            Clase.query.filter_by(shala_id=current_user.shala_id).limit(10).all()
        )
    else:
    ...
    for r in reservas:
        cw.writerow(
            [
                ...
                r.yogui_id,
                r.estado,
                r.fecha_reserva.strftime("%d/%m/%Y %H:%M"),
            ]
        )
...
def reporte_ingresos():
    ...
    if current_user.rol == "ADMIN_SHALA":
        pagos = (
            Pago.query.join(Paquete)
            .filter(Paquete.shala_id == current_user.shala_id)
