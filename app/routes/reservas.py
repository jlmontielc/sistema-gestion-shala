...
def crear_reserva(clase_id):
    ...
    if current_user.saldo_clases < 1:
        flash(
            "⛔ Saldo Insuficiente. No tienes clases disponibles. Por favor, compra un paquete para continuar.",
            "error",
        )
    ...
    total_reservas = Reserva.query.filter_by(
        clase_id=clase_id, estado="RESERVADO"
    ).count()
    ...
    reserva_existente = Reserva.query.filter_by(
        clase_id=clase_id, yogui_id=current_user.id
    ).first()
    ...
    nueva_reserva = Reserva(
        clase_id=clase.id, yogui_id=current_user.id, estado="RESERVADO"
    )
    ...
    # Éxito
    flash(
        f"¡Reserva Confirmada! Se descontó 1 clase. Te quedan: {current_user.saldo_clases} clases.",
        "success",
    )
    return redirect(url_for("clases.listar_clases"))
...
def mis_reservas():
    ...
    mis_reservas = (
        Reserva.query.filter_by(yogui_id=current_user.id)
...
def ver_notificaciones():
    notificaciones = (
        Notificacion.query.filter_by(yogui_id=current_user.id)
...
def marcar_notificaciones_leidas():
    (
        Notificacion.query.filter_by(yogui_id=current_user.id, leida=False).update(
            {"leida": True}
        )
    )
    db.session.commit()
...
def cancelar_reserva(reserva_id):
    ...
    flash(
        f"¡Reserva Cancelada! 🛑 Se devolvió 1 clase a tu cuenta. Saldo actual: {current_user.saldo_clases}.",
        "success",
    )
    return redirect(url_for("reservas.mis_reservas"))